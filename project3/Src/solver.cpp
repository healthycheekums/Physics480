...
{   /*  Velocity-Verlet solver for two coupeled ODEs in a given number of dimensions.
    ...
    int* lostPlanets = new int[integration_points];
    ...
    char *filenameLost = new char[1000];
        // Naming Convention: Planets - Quantity - num_planets - time_step
    ...
    while(time < final_time){
        ...
        for(int nr1=0; nr1<total_planets; nr1++){
            ...
            // Lecture notes 8.3, bottom of page 248
...
double solver::EnergyLoss()
{
    ...
    for(int i=0;i<indices.size();i++) EnergyLossK += all_planets[indices[i]].KineticEnergy();
    return EnergyLossK;
}

void solver::VelocityEuler(int dimension, int integration_points, double final_time, int print_number, double epsilon)
{   /*  Velocity-Euler solver for two coupeled ODEs in a given number of dimensions.
    The algorithm is, exemplified in 1D for position x(t), velocity v(t) and acceleration a(t):
    x(t+dt) = x(t) + dt*v(t);
    v(t+dt) = v(t) - (4*pi*x(t))/(r^3);
    See bottom of page 248 and notes from 2-28-17
    Also see section 8.3.1 (p.247) of lecture notes*/
    ...
    char *filenameB = new char[1000];
    char *filenameLost = new char[1000];
        sprintf(filename, "PlanetsE_%d_%.3f.txt",total_planets,time_step);
        sprintf(filenameE, "PlanetsE_energy_%d_%.3f.txt",total_planets,time_step);
        sprintf(filenameB,"PlanetsEbound_%d_%.3f.txt",total_planets,time_step);
        sprintf(filenameLost,"PlanetsElost_%d_%.3f.txt",total_planets,time_step);
    std::ofstream output_file(filename);
    std::ofstream output_energy(filenameE);
    std::ofstream output_bound(filenameB);
    std::ofstream output_lost(filenameLost);
        ...
        for(int nr1=0; nr1<total_planets; nr1++){
            ...
            //r[total_planets] = 0;
                for(int nr2=nr1+1; nr2<total_planets; nr2++){
                    planet &other = all_planets[nr2];
                    GravitationalForce(current,other,Fxnew,Fynew,Fznew,epsilon);
                    //r[nr2] = distance(current, all_planets[nr2])
                }
            ...
            // Calculate new velocity for current planet
            for(int j=0; j<dimension; j++) current.velocity[j] += (-4*M_PI*M_PI*current.position[j]*time_step);
            // Divided by r^3, but for the sun-earth system, r = 1 AU
        }
